...
.toolbar{position:sticky;top:0;z-index:5;background:rgba(248,240,223,.97);border:1px solid var(--line);border-radius:8px;padding:10px;display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin:16px 0}input[type=search]{flex:1 1 270px;border:1px solid var(--line);border-radius:6px;padding:10px 12px;font-size:14px;background:#fffdf6;color:var(--ink)}button{border:1px solid var(--line);background:#fff8e8;color:var(--ink);border-radius:6px;padding:9px 11px;font-size:13px;cursor:pointer}button.active{background:#261a13;color:#fff;border-color:#261a13}.count{color:var(--muted);font-size:13px;margin-left:auto}
.cards{display:grid;grid-template-columns:repeat(auto-fill,minmax(232px,1fr));gap:10px}.card{--rarity:#9b8053;min-width:0;background:#fffaf0;border:1px solid var(--line);border-left:4px solid var(--rarity);border-radius:8px;padding:10px;display:block;text-align:left;width:100%;min-height:122px}.card:hover,.card:focus{outline:2px solid var(--teal);outline-offset:1px}.card-top{display:grid;grid-template-columns:52px minmax(0,1fr);gap:10px;align-items:center}.card img,.dialog-head img,td img,.mini-item img{object-fit:contain;image-rendering:pixelated}.card img{width:52px;height:52px;background:#eadbc0;border:1px solid #d1ba92;border-radius:6px}.name{display:block;font-weight:800;font-size:14px;overflow-wrap:anywhere}.en{display:block;color:var(--muted);font-size:12px;overflow-wrap:anywhere}.card-note{display:block;margin-top:7px;color:#614b35;font-size:12px;min-height:18px;overflow-wrap:anywhere}.chips{display:flex;flex-wrap:wrap;gap:4px;margin-top:7px}.chip{display:inline-flex;align-items:center;border-radius:999px;background:var(--chip);color:#4b3924;padding:2px 7px;font-size:11px;line-height:18px;white-space:nowrap}.chip.keep{background:#dff1e4;color:#1f643d}.chip.synth{background:#e2eafe;color:#244f9d}.chip.sell{background:#f8e6b8;color:#68470b}.chip.warn{background:#f8d9d5;color:#8b2d25}.market{color:#68470b;background:#f8e6b8}.rarity{width:10px;height:10px;border-radius:50%;display:inline-block;margin-right:5px;border:1px solid rgba(0,0,0,.25)}
table{width:100%;border-collapse:separate;border-spacing:0;background:#fffaf0;border:1px solid var(--line);border-radius:8px;overflow:hidden}th,td{padding:9px 10px;border-bottom:1px solid var(--line);text-align:left;font-size:13px;vertical-align:top}th{background:#ead8b7;color:#4c3928}tr:last-child td{border-bottom:0}td img{width:34px;height:34px;vertical-align:middle;margin-right:8px}.table-wrap{overflow:auto;margin-top:10px}.mini-items{display:flex;flex-wrap:wrap;gap:5px}.mini-item{display:inline-flex;align-items:center;gap:4px;border:1px solid var(--line);border-radius:999px;padding:2px 7px;background:#fffaf0;font-size:12px;white-space:nowrap}.mini-item img{width:22px;height:22px;margin:0}
...
@media(max-width:980px){.status-strip,.grid4,.grid3,.hero-grid,.source-list,.audit-grid{grid-template-columns:1fr 1fr}.detail-grid{grid-template-columns:1fr}}@media(max-width:640px){header{padding:18px 20px}main{padding:14px 20px 36px}.wide-title{display:none}.compact-title{display:inline}.status-strip,.grid4,.grid3,.hero-grid,.source-list,.audit-grid{grid-template-columns:1fr}.toolbar{position:static}.cards{grid-template-columns:1fr}.count{width:100%;margin-left:0}.price-note{grid-template-columns:1fr}.dialog-head{grid-template-columns:56px 1fr auto}.dialog-head img{width:56px;height:56px}h1,.sub,p,li{word-break:break-all;line-break:anywhere}}
"""
...
<header>
  <div class="wrap">
    ...
    <nav class="nav">
      <a href="#rules">判断基準</a><a href="#heroes">キャラクター</a><a href="#tiers">ティア</a><a href="#materials">素材/レベル帯</a><a href="#items">アイテム</a><a href="#market">価格</a><a href="#sources">情報元</a>
    </nav>
  </div>
</header>
<main>
...
const ITEMS=DATA.items, MARKET=DATA.market, HEROES=DATA.heroes, MATERIAL_BANDS=DATA.materialBands, TIER_TABLE=DATA.tierTable, BUILD_TABLE=DATA.buildTable, SOCKET_TABLE=DATA.socketTable, SOURCES=DATA.sources, SOURCE_AUDIT=DATA.sourceAudit;
...
function renderStatusStrip(){const priced=MARKET.filter(m=>String(m.price).includes('¥')).length;const highValue=MARKET[0]?.price||'-';document.getElementById('statusStrip').innerHTML=`<div class="status-card"><b>${ITEMS.length}</b><span>画像付きアイテム</span></div><div class="status-card"><b>${MARKET.length}</b><span>市場掲載アイテム</span></div><div class="status-card"><b>${priced}</b><span>円建て価格あり</span></div><div class="status-card"><b>${esc(highValue)}</b><span>取得中の最高最低価格</span></div>`;document.getElementById('marketUpdated').textContent=DATA.marketUpdatedAt?`最終取得: ${DATA.marketUpdatedAt}`:'価格取得日: 未記録';}
...
function renderCards(){const term=q.value.trim().toLowerCase();const list=ITEMS.filter(it=>passFilter(it)).filter(it=>!term||searchable(it).includes(term));cards.innerHTML=list.map(it=>{const market=it.market.slice(0,2).map(m=>`<span class="chip market"><span class="rarity" style="background:#${esc(m.color||'ddd')}"></span>${esc(m.price)}</span>`).join('');const note=it.tierBand||it.uses[0]||it.sellAdvice;return `<button class="card" data-id="${it.id}" style="--rarity:${rarityColor[it.grade]||'#a8894c'}"><span class="card-top"><img src="${esc(it.icon)}" alt="${esc(it.ja)}" loading="lazy"><span><span class="name">${esc(it.ja)}</span><span class="en">${esc(it.en)} / ID ${it.id}</span></span></span><span class="chips"><span class="chip ${actionClass(it.action)}">${esc(it.action)}</span><span class="chip">${esc(it.category)}</span>${gradeChip(it.grade)}${it.tags.slice(0,3).map(t=>`<span class="chip">${esc(t)}</span>`).join('')}${market}</span><span class="card-note">${esc(note)}</span></button>`;}).join('');count.textContent=`${list.length} / ${ITEMS.length}件`;}
...
renderStatusStrip();renderHeroCards();renderTierCards();renderBuildCards();renderSocketTable();renderMaterialTable();renderCards();renderMarket();renderAuditCards();renderSources();
</script>
</body>
</html>
"""
