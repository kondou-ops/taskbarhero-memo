        ...
        heroes.append(
            {
                ...
                "ja": names[hero_id]["ja"],
                "en": names[hero_id]["en"],
                "icon": HERO_ICON_BY_ID.get(hero_id, ""),
...
.effect-goals{display:grid;grid-template-columns:repeat(auto-fill,minmax(132px,1fr));gap:8px;margin:12px 0}.effect-goal{min-width:0;text-align:left;background:#fffaf0;border:1px solid var(--line);border-left:4px solid var(--teal);border-radius:8px;padding:9px 10px}.effect-goal b{display:block;font-size:13px}.effect-goal span{display:block;color:var(--muted);font-size:12px;margin-top:3px}.effect-goal.active{background:#241a14;color:#fff;border-color:#241a14}.effect-goal.active span{color:#e4d2b6}.slot-stats{display:flex;flex-wrap:wrap;gap:6px;margin:8px 0 12px}.slot-stats span{border:1px solid var(--line);border-radius:999px;background:#fffaf0;padding:4px 9px;font-size:12px}.effect-table-wrap{max-height:calc(100vh - 190px);min-height:420px}.effect-search-table th{top:0;box-shadow:0 2px 0 rgba(74,52,37,.16)}.effect-search-table th:nth-child(2),.effect-search-table td:nth-child(2){min-width:58px;white-space:nowrap}.effect-search-table th:nth-child(3),.effect-search-table td:nth-child(3){min-width:118px}.effect-sort{display:inline-flex;align-items:center;gap:5px;border:1px solid var(--line-dark);background:#fff8e8;color:#4c3928;border-radius:6px;padding:5px 7px;font-weight:800}.effect-hit{background:#e5f2e2}.focus-badges{display:flex;flex-wrap:wrap;gap:4px;margin-top:6px}.focus-badge{display:inline-flex;border-radius:999px;background:#dff1e4;color:#1f643d;padding:1px 6px;font-size:11px;line-height:17px}.effect-empty{padding:18px;color:var(--muted)}
...
@media(max-width:980px){.grid4,.grid3,.hero-grid,.source-list,.audit-grid,.effect-goals{grid-template-columns:1fr 1fr}.detail-grid{grid-template-columns:1fr}}@media(max-width:640px){html,body{width:100%;max-width:100%;overflow-x:clip}body{background:linear-gradient(180deg,#151017 0,#261a13 520px,#f0e5cf 521px)}header{width:100%;max-width:100%;overflow:hidden;padding:12px 14px}header .wrap{width:100%;max-width:100%;min-width:0;margin:0}.nav{display:grid;width:100%;max-width:100%;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;overflow:hidden}.nav a{display:flex;align-items:center;justify-content:center;min-width:0;max-width:100%;text-align:center;font-size:12px;line-height:1.25;padding:7px 5px;white-space:normal;overflow-wrap:anywhere}main{width:100%;max-width:100%;min-width:0;margin:0;padding:14px 16px 36px;overflow-x:hidden}.wide-title{display:none}.compact-title{display:inline}.sub,.section-note{display:block;width:100%;max-width:100%;min-width:0;white-space:normal;overflow-wrap:anywhere;word-break:break-all;line-break:anywhere}.grid4,.grid3,.hero-grid,.source-list,.audit-grid{width:100%;max-width:100%;grid-template-columns:minmax(0,1fr)}.panel,.status-card,.hero-card,.item-workbench,.price-note,.table-wrap,.scroll-table,.material-band{max-width:100%;min-width:0}.scroll-table,.table-wrap{width:calc(100% + 16px);max-width:calc(100% + 16px);margin-left:-8px;margin-right:-8px;-webkit-overflow-scrolling:touch}.panel{overflow:hidden}.toolbar{position:static}.cards{grid-template-columns:minmax(0,1fr)}input[type=search],select{width:100%;flex:1 1 100%}.count{width:100%;margin-left:0}.price-note{grid-template-columns:1fr}.item-scroll{max-height:68vh}.dialog-head{grid-template-columns:56px 1fr auto}.dialog-head img{width:56px;height:56px}main>h2{color:var(--ink);background:rgba(248,240,223,.96);border:1px solid var(--line);border-radius:8px;padding:10px 12px;margin:20px 0 8px}main>h2+.section-note{color:#5d4c39;background:rgba(248,240,223,.94);border:1px solid var(--line);border-radius:8px;padding:8px 10px;margin:0 0 10px}.effect-goals{display:flex;width:100%;max-width:100%;grid-template-columns:none;gap:8px;margin:10px 0 12px;padding:2px 4px 10px 2px;overflow-x:auto;overscroll-behavior-x:contain;-webkit-overflow-scrolling:touch}.effect-goal{flex:0 0 136px;min-height:58px}.effect-table-wrap{max-height:62vh;min-height:360px;width:calc(100% + 16px);max-width:calc(100% + 16px);margin-left:-8px;margin-right:-8px;-webkit-overflow-scrolling:touch}.effect-table-wrap table{min-width:720px}.effect-search-table th,.effect-search-table td{font-size:12px;padding:8px}.keep-icon-grid{grid-template-columns:repeat(auto-fill,minmax(48px,1fr));gap:8px;padding:10px}.keep-tip{position:fixed;left:16px;right:16px;top:92px;bottom:auto;transform:none;width:auto;max-width:none}h1,.sub,p,li{word-break:break-all;line-break:anywhere}}
"""
...
function renderHeroCards(){document.getElementById('heroCards').innerHTML=HEROES.map((h,i)=>`<a class="hero-card ${i%3===0?'keep':i%3===1?'synth':'warn'}" href="${esc(h.page)}"><span class="rank">${esc(h.role)}</span><h3>${esc(h.ja)}</h3>${h.icon?`<div class="hero-visual"><img src="${esc(h.icon)}" alt=""></div>`:''}<p class="loadout">${esc(h.gear)}</p><div class="skill-line">${h.skills.map(s=>`<span class="chip">${esc(s.ja)}</span>`).join('')}</div><p class="small" style="margin-top:10px">個別ページを見る</p></a>`).join('');}
...
def keep_material_icon(item: dict) -> str:
    ...
    if item.get("effectText"):
        details.append(item["effectText"])
    elif item.get("uses"):
        details.extend(item["uses"][:2])
    detail_html = "".join(f"<span>{h(detail)}</span>" for detail in details if detail)
    return f'<button class="keep-icon" type="button"><img src="{h(item["icon"])}" alt=""><span class="keep-tip"><b>{h(item["ja"])}</b>{detail_html}</span></button>'
...
EFFECT_FOCUS = [
    ...
    },
    {
        ...
        "label": "火ダメージ",
        "short": "火",
        "terms": ("火ダメージ", "Fire Damage"),
    },
    {
        "id": "coldDamage",
        "label": "冷気ダメージ",
        "short": "冷気",
        "terms": ("冷気ダメージ", "Cold Damage"),
    },
    {
        "id": "lightningDamage",
        "label": "雷ダメージ",
        "short": "雷",
        "terms": ("雷ダメージ", "Lightning Damage"),
    },
    {
        "id": "critChance",
        "label": "クリティカル率",
        "short": "クリ率",
        "terms": ("クリティカル率", "Crit Chance"),
    },
    {
        "id": "critDamage",
        "label": "クリティカルダメージ",
        "short": "クリダメ",
        "terms": ("クリティカルダメージ", "Crit Damage"),
    },
    {
        "id": "castSpeed",
        "label": "詠唱速度",
        "short": "詠唱",
        "terms": ("詠唱速度", "Cast Speed"),
    },
    {
        "id": "cooldown",
        "label": "クールダウン短縮",
        "short": "短縮",
        "terms": ("クールダウン短縮", "Cooldown Reduction"),
    },
    {
        "id": "maxHp",
        "label": "最大体力",
    ...
    },
    {
        ...
        "label": "火耐性",
        "short": "火耐性",
        "terms": ("火耐性", "Fire Resistance"),
    },
    {
        "id": "coldResist",
        "label": "冷気耐性",
        "short": "冷耐",
        "terms": ("冷気耐性", "Cold Resistance"),
    },
    {
        "id": "lightningResist",
        "label": "雷耐性",
        "short": "雷耐",
        "terms": ("雷耐性", "Lightning Resistance"),
    },
    {
        "id": "chaosResist",
        "label": "混沌耐性",
        "short": "混沌",
        "terms": ("混沌耐性", "Chaos Resistance"),
    },
    {
        "id": "allResist",
        "label": "全属性耐性",
        "short": "全耐",
        "terms": ("全属性耐性", "All Elemental Resistance"),
    },
    {
        "id": "damageReduction",
        "label": "ダメージ軽減",
        "short": "軽減",
        "terms": ("ダメージ軽減", "Damage Reduction"),
    },
    {
        "id": "damageAbsorb",
        "label": "ダメージ吸収",
        "short": "吸収",
        "terms": ("ダメージ吸収", "Damage Absorption"),
    },
    {
        "id": "block",
        "label": "ブロック率",
        "short": "ブロック",
        "terms": ("ブロック率", "Block Chance"),
    },
    {
        "id": "dodge",
        "label": "回避率",
        "short": "回避",
        "terms": ("回避率", "Dodge Chance"),
    },
    {
        "id": "hpPerHit",
        "label": "ヒット時体力回復",
        "short": "ヒット回復",
        "terms": ("ヒット時体力回復", "HP per Hit"),
    },
    {
        "id": "hpRegen",
        "label": "秒間体力回復",
        "short": "秒回復",
        "terms": ("秒間体力回復", "HP Regen/sec"),
    },
    {
        "id": "lifeLeech",
        "label": "ライフ吸収",
        "short": "吸血",
        "terms": ("ライフ吸収", "Life Leech"),
    },
    {
        "id": "multistrike",
        "label": "連続攻撃",
        "short": "連続",
        "terms": ("連続攻撃", "Multistrike"),
    },
    {
        "id": "projectile",
        "label": "投射物数",
        "short": "投射",
        "terms": ("投射物数", "Projectile Count"),
    },
    {
        "id": "basicAttack",
        "label": "基本攻撃回数",
        "short": "基本攻撃",
        "terms": ("基本攻撃回数", "Basic Attack Count"),
    },
]
            ...
            rows.append(
                {
                    ...
                    "gradeRank": GRADE_ORDER.index(row.get("Grade", item.get("grade", "Common")))
                    if row.get("Grade", item.get("grade", "Common")) in GRADE_ORDER
                    else 0,
...
def effects_page_template(data: dict) -> str:
    ...
    goal_options = "\n".join(f'        <option value="{h(goal["id"])}">{h(goal["label"])}</option>' for goal in EFFECT_FOCUS)
    body = f"""<!doctype html>
<html lang="ja">
<head>
<meta charset="utf-8">
...
<main>
  <section class="panel" id="finder">
    <h2>目的で探す</h2>
    <p class="section-note">防具で移動速度を探す場合は、部位を「防具」、目的を「移動速度」にします。等級は初期表示で高い順。</p>
...
{goal_options}
  ...
  <div class="scroll-table market-scroll effect-table-wrap">
    <table class="effect-search-table">
      <thead><tr><th>素材</th><th>種類</th><th><button class="effect-sort" id="gradeSortBtn" type="button">等級 高い順</button></th><th>武器</th><th>防具</th><th>アクセサリー</th></tr></thead>
...
function sortRows(a,b){{const rank=gradeSort==='desc'?b.gradeRank-a.gradeRank:a.gradeRank-b.gradeRank;if(rank)return rank;return a.ja.localeCompare(b.ja,'ja');}}
...
function renderGoalCards(){{const base=filteredBase();const cards=[{{id:'all',label:'全効果'}}].concat(GOALS);document.getElementById('goalCards').innerHTML=cards.map(g=>{{const n=g.id==='all'?base.length:base.filter(r=>rowHasGoal(r,g.id,'all')).length;return `<button class="effect-goal ${{goalEl.value===g.id?'active':''}}" type="button" data-goal="${{esc(g.id)}}"><b>${{esc(g.label)}}</b><span>${{n}}件</span></button>`;}}).join('');document.querySelectorAll('.effect-goal').forEach(btn=>btn.addEventListener('click',()=>{{goalEl.value=btn.dataset.goal;renderFinder();}}));}}
...
function renderFinder(){{const base=filteredBase();const rows=base.filter(rowPasses).sort(sortRows);renderGoalCards();renderSlotStats(base);sortBtn.textContent=gradeSort==='desc'?'等級 高い順':'等級 低い順';document.getElementById('finderCount').textContent=`${{rows.length}}件`;document.getElementById('finderRows').innerHTML=rows.length?rows.map(r=>`<tr><td>${{itemCell(r)}}</td><td>${{esc(r.kind)}}</td><td>${{gradeChip(r.grade)}}</td>${{SLOTS.map(s=>effectCell(r,s)).join('')}}</tr>`).join(''):`<tr><td colspan="6" class="effect-empty">該当する素材はありません。</td></tr>`;}}
[kindEl,slotEl,goalEl,qEl].forEach(el=>el.addEventListener(el===qEl?'input':'change',renderFinder));
sortBtn.addEventListener('click',()=>{{gradeSort=gradeSort==='desc'?'asc':'desc';renderFinder();}});
renderFinder();
</script>
</body>
</html>
...
<main>
  <section class="detail-grid">
    ...
    <div class="panel synth">
      <div class="hero-visual hero-detail-visual">{f'<img src="{h("../" + hero["icon"])}" alt="">' if hero.get("icon") else ""}</div>
    </div>
  </section>
